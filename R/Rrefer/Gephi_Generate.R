...
dataSet <- read.table("c:\\gephi\\apriori_key.txt", header = FALSE, sep = ",",encoding = "utf-8")

#dataSet <- read.csv("c:\\gephi\\apriori_vw.txt", header = FALSE, sep = "\t")
...
write.gexf(nodes = ndt, edges = edges_df, nodesAtt = nodes_att, edgesAtt = edges_att,nodesVizAtt = nodes_att_viz, 
           edgesVizAtt = edges_att_viz, 
                  defaultedgetype = "undirected", output = "c:\\gephi\\key1_apriori.gexf")
