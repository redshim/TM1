...
#dataSet <- read.table("c:\\gephi\\apriori_key.txt", header = FALSE, sep = ",")
#dataSet <- read.table("c:\\gephi\\key1.csv", header = FALSE, sep = "\t" ,encoding = "utf-8")
dataSet <- read.table("c:\\gephi\\key1\\key_apr1.csv", header = FALSE, sep = ",", encoding = "UTF-8")
...
write.gexf(nodes = nodes_df, edges = edges_df, nodesAtt = nodes_att, edgesAtt = edges_att, nodesVizAtt = nodes_att_viz, edgesVizAtt = edges_att_viz, defaultedgetype = "undirected", output = "c:\\Gephi\\apriori_key1.txt.gexf",encoding='UTF-8')
